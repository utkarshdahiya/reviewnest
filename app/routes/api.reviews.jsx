...
export async function loader({ request }) {
  ...
  return new Response(JSON.stringify({ reviews, summary, settings }), { headers: { "Content-Type": "application/json", ...corsHeaders } });
}
...
export async function action({ request }) {
  ...
  try {
    ...
    if (!productId || !authorName || !rating || !body) return new Response(JSON.stringify({ error: "Missing required fields" }), { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } });
    const review = await createReview({ shop, productId, authorName, rating, title, body, imageUrl });
    return new Response(JSON.stringify({ ok: true, review }), { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders } });
  } catch (e) {
